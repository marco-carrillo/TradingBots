...
// Getting  of the strategy parametrs form local storage

var edit_index = JSON.parse(localStorage.getItem("edit-strategy"));
editArray = edit_index.index

// console.log(edit_index);
$("#strat-name").val(edit_index[editArray].name);
$("#strat-desc").val(edit_index[editArray].desc);
if ($("#adx-ok").is(":checked")) { total_adx = parseInt($("#adx-slider").val(edit_index[editArray].adx_wgth)) }
if ($("#rsi-ok").is(":checked")) { total_rsi = parseInt($("#rsi-slider").val(edit_index[editArray].rsi_wgth)) }
if ($("#srsi-ok").is(":checked")) { total_srsi = parseInt($("#srsi-slider").val(edit_index[editArray].srsi_wgth)) }
if ($("#mom-ok").is(":checked")) { total_mom = parseInt($("#mom-slider").val(edit_index[editArray].mom_wgth)) }
...
function editStrategy() {
    ...
    if (total_pctg === 100 && strat_name.length > 0 && strat_name.length <= 10 && strat_desc.length > 0 && strat_desc.length <= 50) {
        ...
        all_strategies[edit_index.index] = edited_strategy;
        localStorage.setItem("strategies", JSON.stringify(all_strategies));

        $.confirm({
            title: 'Success!!!!!',
            content: 'Your strategy ' + strat_name + ' was edited!!! Please, push the back button to go back',
            type: 'green',
            typeAnimated: true,
            buttons: {
                tryAgain: {
        ...
        // setting the edited strategy name
        var error_message = "";
...
//  Assign the function to add when the safe button is clicked
$("#edit-stg").on("click", editStrategy);
